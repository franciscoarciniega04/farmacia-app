// src/screens/ventas/ModalAgregarProducto.js
...
export default function ModalAgregarProducto({ visible, onClose, onAgregar }) {
  ...
  const handleAgregar = () => {
    ...
    if (cantidadNum > productoSeleccionado.stock) {
      Alert.alert('Error', `Stock insuficiente (Disponible: ${productoSeleccionado.stock})`);
      return;
    }
  ...
  const renderProductoItem = ({ item }) => (
    ...
    >
      ...
      <Text style={styles.productoItemPrecio}>${item.precioVenta.toFixed(2)}</Text>
    </TouchableOpacity>
  );
