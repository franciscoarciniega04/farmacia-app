// src/screens/ajustes/GestionCategoriasScreen.js
...
export default function GestionCategoriasScreen({ navigation }) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      const url = categoriaEditar
        ? `${API_BASE_URL}/categorias/${categoriaEditar.idCategoria}`
      ...
      if (!resp.ok) throw new Error('Error al guardar categoría');
          ...
          onPress: async () => {
            try {
              const resp = await fetch(
                `${API_BASE_URL}/categorias/${categoria.idCategoria}`,
                { method: 'DELETE' }
              );
  ...
  const renderCategoriaItem = ({ item }) => (
    <View style={styles.categoriaCard}>
      <View style={styles.iconContainer}>
        <Ionicons name="pricetags" size={32} color="#dc3545" />
      </View>
