// src/screens/ajustes/GestionClientesScreen.js
...
export default function GestionClientesScreen({ navigation }) {
  ...
  const cargarClientes = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${API_BASE_URL}/clientes/?soloActivos=true`);
  ...
  const handleGuardar = async () => {
    ...
    try {
      const url = clienteEditar
        ? `${API_BASE_URL}/clientes/${clienteEditar.idCliente}`
      ...
      if (!resp.ok) throw new Error('Error al guardar cliente');
          ...
          onPress: async () => {
            try {
              const resp = await fetch(
                `${API_BASE_URL}/clientes/${cliente.idCliente}`,
                { method: 'DELETE' }
              );
  ...
  const renderClienteItem = ({ item }) => (
    <View style={styles.clienteCard}>
      <View style={styles.clienteInfo}>
        ...
        {item.email && (
          <Text style={styles.clienteDetalle}>✉️ {item.email}</Text>
        )}
      </View>
